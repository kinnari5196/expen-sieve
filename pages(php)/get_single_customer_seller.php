...
$grup_id=1;
$sel="SELECT * FROM customer_seller where business_id=".$bid." AND isactive=".$is." AND type='".$type."' AND cs_id=".$cusid.";";
$x=0;
$rs=mysqli_query($conn,$sel);
                                ...
                                if($rs22221)
                                {
                                    if(mysqli_num_rows($rs22221)>0)
                                    {
                                        while ($data22221=mysqli_fetch_assoc($rs22221)) 
                                        {
                                            ...
                                            $date=$data22221["date_since"];
                                        }
                                    }
                                }

                 $data11[$x] = array("id" => stripslashes($data["cs_id"]),"date" => stripslashes($date), "name" => stripslashes($data["name"]), "address" => stripslashes($data["addressline"]), "pincode" => stripslashes($data["pincode"]), "gstNo" => stripslashes($data["gst_no"]),"phone1" => stripslashes($phone1),"phone2" => stripslashes($phone2),"area" => stripslashes($areaname),"city" => stripslashes($cityname),"state" => stripslashes($statename),"currentBalance" => stripslashes($balance));
	...
	$output= array('customer_seller'=>$data11);
